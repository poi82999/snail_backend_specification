...
NEW_CONTENT = '''IMPLEMENTATION_GUIDES = {
    "1": [
...
SCREEN_PLAYBOOKS = {
    "1": {
        "api_sequence": [
            ...
            "[회원가입 흐름]: 회원가입 폼을 다 채우고 제출하면 `POST /owner/auth/register`를 호출합니다. 성공(200) 응답을 받으면 지체 없이 '사업자 인증 서류 제출 화면'으로 넘겨주시면 됩니다.",
...
# Replace IMPLEMENTATION_GUIDES and SCREEN_PLAYBOOKS
pattern = re.compile(r'IMPLEMENTATION_GUIDES = \{.*?\n\}\n\n\nSCREEN_PLAYBOOKS = \{.*?\n\}\n', re.DOTALL)
new_content = pattern.sub(NEW_CONTENT + '\n', content)

with open('tools/build_owner_webapp_index.py', 'w', encoding='utf-8') as f:
    f.write(new_content)
print("owner_webapp playbooks rewritten successfully.")
